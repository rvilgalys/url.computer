...
describe("HostnameEditor", () => {
  ...
  it("should render read-only mode when not editable", () => {
    render(<HostnameEditor hostname="example.com" isEditable={false} />);
  ...
  it("should show error message for invalid hostname", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Domain cannot contain consecutive dots")
      ).toBeInTheDocument();
    });
  });
  ...
  it("should clear error when user fixes hostname", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Domain cannot contain consecutive dots")
