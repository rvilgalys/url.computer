...
describe("PathEditor", () => {
  ...
  it("should render read-only mode when not editable", () => {
    render(<PathEditor pathname="/api/users" isEditable={false} />);
  ...
  it("should show error message for invalid pathname", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Path contains invalid characters")
      ).toBeInTheDocument();
    });
  });
  ...
  it("should clear error when user fixes pathname", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Path contains invalid characters")
  ...
  it("should show breadcrumb navigation for complex paths", () => {
    ...
    expect(
      screen.getByText(
        (content, element) => content.includes("api") && content.includes("→")
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (content, element) => content.includes("v1") && content.includes("→")
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (content, element) => content.includes("users") && content.includes("→")
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (content, element) => content.includes("123") && content.includes("→")
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (content, element) =>
          content.includes("posts") && !content.includes("→")
      )
    ).toBeInTheDocument();
  });
