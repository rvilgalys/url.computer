...
export default function HeadersEditor({
  ...
  useEffect(() => {
    const headerEntries = Object.entries(headers).map(([key, value]) => {
      ...
      const existingParam = paramsRef.current.find(
    ...
    const currentParamsSerialized = JSON.stringify(
      paramsRef.current.map(({ key, value }) => ({ key, value }))
    );
    const newParamsSerialized = JSON.stringify(
  ...
  const updateParent = (newParams: HeaderParam[]) => {
    ...
    newParams.forEach((param) => {
      if (param.key.trim()) {
        // Only add params with non-empty keys
  ...
  return (
    <div className="mt-4">
      ...
      <div className="space-y-2">
        ...
        <button
          onClick={handleAddParam}
          className="text-left p-2 text-elf-yellow hover:text-elf-orange hover:bg-elf-mid-blue/20 rounded-md transition-colors font-medium"
        >
          + Add Header
        </button>
      </div>
